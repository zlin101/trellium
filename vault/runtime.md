...
- TASK-0011 M1 verdict **No-Go** (2026-09-15): both arms zero key omissions / zero corrections / zero hard-metric violations across all three scenarios (A0's AGENTS.md+vault floor suffices — floor effect); A1 costs more (visible +79% median). Structure facts: project-level trellium-work via `.claude/skills/` was NOT discovered by Codex (Codex's actual project-level discovery location unverified — `.claude/skills` is a Claude Code path) and the `agent-task` global leak reproduced on Codex. Per stop conditions only the template-leak fix remains; M2/M3 project-skill items cancelled.
