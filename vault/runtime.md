...
| TASK-0013 | Close the adoption durability and local Git-boundary false-health gap. | active | M0-M5 done: ablation Go, HEAD/local-boundary gates shipped, 2026.09.8 synced (149 tests); next independent review, then owner acceptance. |
...
- TASK-0013: active — M0-M5 complete (2026-09-18): M0 prereg + 6 red tests committed before any product change; M1 ablation (4 fresh sessions, contract-gate **Go** with recorded reservations); M2/M3 shipped the HEAD durability gate (`CORE_STORAGE_UNCOMMITTED`/`CORE_STORAGE_IGNORED`/`CORE_STORAGE_UNVERIFIED`) and local boundary checks (`LOCAL_BOUNDARY_UNCONFIGURED`/`LOCAL_BOUNDARY_OVERREACH`); M4 landed the bilingual adoption-completion contract and the `generated ≠ durable` adopt output; M5 synced 2026.09.8 (VERSION/MIGRATIONS/README/snapshots) and recorded D-0010. Self-check's single error is the true-positive on the owner's uncommitted `docs/engineering/` core. Next: independent review, owner acceptance.
...
- TASK-0013 M0-M5 implemented (2026-09-18): P0/P1 ablation Go (contract gate), HEAD durability + local boundary checks, bilingual adoption completion contract, `generated ≠ durable` adopt output, 2026.09.8 version/migrations/README/snapshot sync, D-0010; suite 149 tests. Awaiting independent review and owner acceptance.
