...
### 修订 1（2026-09-08，M2 判断 cell 执行方式；owner 指令"你开始做呗"授权）

- 投放人由 owner 改为 **GLM 经隔离子代理会话执行**（每个 cell 一个全新子代理，无聊天历史）；提示词材料与 prompts.md 登记版一致，另按本修订做封闭书调整。
- **封闭书调整**：材料中的事实清单是假设仓库的权威描述，判断只依据材料本身；指示被测会话不读取任何仓库文件（尤其 docs/evals/），以防真实仓库差异与跨 cell 材料泄漏。此调整同时适用于三个 cell，不改变 E0/E1/E2 的信息结构差异。
