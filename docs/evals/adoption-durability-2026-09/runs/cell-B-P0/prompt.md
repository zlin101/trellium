...
- 接入后的项目文件树：<host>/docs/evals/adoption-durability-2026-09/materials/fixture-b（目录内为接入后的实际文件内容）
- 接入结束后 `git status --porcelain` 的逐字输出：

- 另附 `git status --porcelain --ignored` 的逐字输出：
!! vault/decisions/
!! vault/details/
!! vault/tasks/
- `adopt` 命令的完整终端输出（逐字）：
...
changed: 11
  ...
  - vault/project.md
  - vault/runtime.md
next: ask your Agent to read AGENTS.md, vault/index.md (cheat sheet), and vault/runtime.md; read vault/governance.md in full for Level B/C work
